...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/10 16:40:47 by yde-goes          #+#    #+#             */
/*   Updated: 2024/02/17 15:14:17 by yde-goes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void BitcoinExchange::_loadDatabase(void) {
  ...
  if (line != DATABASE_HEADER) throw InvalidFileHeaderException();
...
void BitcoinExchange::readInputFile(std::string filename) {
  ...
  std::getline(file, line);
  if (line != INPUT_HEADER) throw InvalidFileHeaderException();
  while (std::getline(file, line)) {
    try {
      date = line.substr(0, line.find('|'));
      value = line.substr(line.find('|') + 1);
...
void BitcoinExchange::_outputExchangeCalculus(std::string date, float value) {
  ...
  if (it != this->_database.end()) {
    std::cout << GREEN << date << " => " << value << " = " << value * it->second
              << RESET << std::endl;
    return;
  }
  ...
  std::cout << GREEN << date << " => " << value << " = "
            << value * closest_value << RESET << std::endl;
}
